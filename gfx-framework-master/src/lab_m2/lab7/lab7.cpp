...
void Lab7::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix)
{
    ...
    int loc_bones_matrix_array = shader->GetUniformLocation("Bones");
    glUniformMatrix4fv(loc_bones_matrix_array, 200, GL_FALSE, glm::value_ptr(bones[0]));
...
void Lab7::ComputeInterpolatedScaling(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    // Save the result in the out variable
    out = (1 - factor) * pNodeAnim->mScalingKeys[scalingIndex].mValue + factor * pNodeAnim->mScalingKeys[nextScalingIndex].mValue;
}
...
void Lab7::ComputeInterpolatedPosition(aiVector3D& out, float animationTime, const aiNodeAnim* pNodeAnim)
{
    ...
    // Save the result in the out variable
    out = (1 - factor) * pNodeAnim->mPositionKeys[positionIndex].mValue + factor * pNodeAnim->mPositionKeys[nextPositionIndex].mValue;
}
