...
void main()
{
    ...
    if (instance[0] < no_of_instances)
    {
        ...
        */
        ...
        float bezier_step = 1.0f/no_of_generated_points;
        float translate_step =  max_translate/no_of_instances;
        float rotation_step = max_rotate/no_of_instances;

        for (float t = 0.0f; t <= 1.01f; t += bezier_step)
        {
            if (do_rotation == 1)
            {
                gl_Position = Projection * View * vec4(rotateY(bezier(t), instance[0] * rotation_step), 1);   
                EmitVertex();
            
                gl_Position = Projection * View * vec4(rotateY(bezier(t), (instance[0] + 1) * rotation_step), 1);   
                EmitVertex();
            }
            else
            {
                gl_Position = Projection * View * vec4(translateX(bezier(t), instance[0] * translate_step), 1);   
                EmitVertex();
            
                gl_Position = Projection * View * vec4(translateX(bezier(t), (instance[0] + 1) * translate_step), 1);   
                EmitVertex();
            }
        }
