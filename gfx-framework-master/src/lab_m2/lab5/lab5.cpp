...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    // Lighting pass
    {
        ...
        for (auto& lightInfo : lights)
        {
            ...
            int loc_light_position = shader->GetUniformLocation("light_position");
            glUniform3f(loc_light_position, lightInfo.position.x, lightInfo.position.y, lightInfo.position.z);

            int loc_light_color = shader->GetUniformLocation("light_color");
            glUniform3f(loc_light_color, lightInfo.color.x, lightInfo.color.y, lightInfo.color.z);

            int loc_light_radius = shader->GetUniformLocation("light_radius");
            glUniform1f(loc_light_radius, lightInfo.radius);
